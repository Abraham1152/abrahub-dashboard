...
export default function LeadsPage() {
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter(lead => {
      ...
      if (utmFilter !== 'all' && (lead.utm_source || 'organic') !== utmFilter) return false
      return true
    })
  }, [leads, search, tempFilter, statusFilter, utmFilter])
  ...
  const utmSources = useMemo(() => {
    const sources: Record<string, { leads: number; converted: number; revenue: number }> = {}
    for (const l of leads) {
      ...
      if (l.status === 'converted') {
        ...
        sources[src].revenue += l.conversion_value || 0
      }
    }
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {/* UTM Attribution Widget */}
      {utmSources.length > 0 && (
        <div className="card p-4">
          <p className="text-xs font-semibold text-gray-500 dark:text-neutral-500 uppercase tracking-wider mb-3">Atribuição por Origem</p>
          <div className="flex flex-wrap gap-2">
            {utmSources.map(([src, data]) => (
              <button
                key={src}
                onClick={() => setUtmFilter(utmFilter === src ? 'all' : src)}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs border transition-all ${
                  utmFilter === src
                    ? 'bg-orange-500 text-white border-orange-500 shadow-md shadow-orange-500/20'
                    : 'bg-white dark:bg-neutral-800 border-gray-200 dark:border-neutral-700 text-gray-700 dark:text-neutral-300 hover:border-orange-300 dark:hover:border-orange-500/40'
                }`}
              >
                <span className="font-semibold">{src}</span>
                <span className={utmFilter === src ? 'opacity-80' : 'text-gray-500 dark:text-neutral-500'}>{data.leads} lead{data.leads !== 1 ? 's' : ''}</span>
                {data.converted > 0 && (
                  <span className={utmFilter === src ? 'opacity-90' : 'text-green-600 dark:text-green-400'}>
                    · {data.converted} conv
                  </span>
                )}
                {data.revenue > 0 && (
                  <span className={`font-bold ${utmFilter === src ? 'opacity-90' : 'text-emerald-700 dark:text-emerald-300'}`}>
            ...
            {leads.map(lead => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  ...
                  {lead.utm_source && (
                    <span className="mt-0.5 inline-block px-1.5 py-0.5 rounded text-[9px] font-medium bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-300">
                      {lead.utm_source}
                    </span>
                  )}
                </td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-neutral-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-6">
          ...
          {/* UTM Attribution */}
          {(lead.utm_source || lead.utm_medium || lead.utm_campaign || lead.utm_term || lead.utm_content || lead.utm_ref) && (
            <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20">
              <p className="text-sm font-semibold text-blue-700 dark:text-blue-300 mb-3">Atribuição UTM</p>
              <div className="grid grid-cols-2 gap-2 text-xs">
                {([
                  ...
                  <div key={label}>
                    <p className="text-blue-600 dark:text-blue-400 font-medium">{label}</p>
                    <p className="text-blue-700 dark:text-blue-300 font-mono text-[10px] break-all">{value}</p>
                  </div>
                ))}
              </div>
            </div>
